...
class AppInvoice extends Model
{
    ...
    {
        //Verfificando a $filter no indice[status] se não está vaia, e depois se esta como paga recebe um parametro de pago, se não de não paga, e se nenhum condição for atendida retorna null
        ...
        //Verfificando a $filter no indice[category] se não esta vazia e se catergory for diferente do indice all, ele recebe de fato o category, caso nenhuma condiçaõ atendida retorna null
        $category = (!empty($filter["category"]) && $filter["category"] != "all" ? "AND category_id = '{$filter["category"]}'" : null);
        ////Verfificando a $filter no indice[date] se não esta vazia, dividino a string com explode, pelo sepaador "-", e pegar o indice 1, caso o contrario retorna direto o ano q eu estou.
